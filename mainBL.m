...
Mz=stblrnd(alphaz,betaz,h^(1/alphaz),0,1,Nz);
Bh=sqrt(h)*randn(3,Nz);
zxf=10*(z0y-z0x)*h+(1+z0z).*Bh(1,:)+Bh(2,:)+sigmax*Mx;
zxf0=z0x;
zyf=(4*z0x-z0y-z0x.*z0z)*h+z0y.*Bh(2,:)+sigmay*My;
zyf0=z0y;
zzf=(-8/3*z0z+z0x.*z0y)*h+z0x.*Bh(3,:)+sigmaz*Mz;
zzf0=z0z;

%%% Identify alphax and sigmax
q=epsilong;
m=5;
N=2;
nkp=zeros(1,N+1);
nkn=zeros(1,N+1);
for k=0:N
    I=((zxf)>=m^k*q)&((zxf)<m^(k+1)*q);
    nkp(k+1)=length(zxf(I));
    I=((zxf)<=-m^k*q)&((zxf)>-m^(k+1)*q);
    nkn(k+1)=length(zxf(I));
end
nk=nkp+nkn;
nratio=nk(1)./nk(2:end);
pos=1:N;
alphax1=log(nratio)./(pos*log(m));
alphax0=sum(alphax1)/N;
...
pos=0:N;
sigmakx=q*m.^pos.*(nk*alphax0/(h*Nz*kalphax*(1-m^(-alphax0)))).^(1/alphax0);
sigmax0=sum(sigmakx)/(N+1);

%%% Identify alphay and sigmay
nkp=zeros(1,N+1);
nkn=zeros(1,N+1);
for k=0:N
    I=((zyf)>=m^k*q)&((zyf)<m^(k+1)*q);
    nkp(k+1)=length(zyf(I));
    I=((zyf)<=-m^k*q)&((zyf)>-m^(k+1)*q);
    nkn(k+1)=length(zyf(I));
end
nk=nkp+nkn;
nratio=nk(1)./nk(2:end);
pos=1:N;
alphay1=log(nratio)./(pos*log(m));
alphay0=sum(alphay1)/N;
% nratio=nk(1:end-1)./nk(2:end);
% alpha1=log(nratio)./(log(m));
% alpha0=sum(alpha1)/N;
...
pos=0:N;
sigmaky=q*m.^pos.*(nk*alphay0/(h*Nz*kalphay*(1-m^(-alphay0)))).^(1/alphay0);
sigmay0=sum(sigmaky)/(N+1);

%%% Identify alphaz and sigmaz
nkp=zeros(1,N+1);
nkn=zeros(1,N+1);
for k=0:N
    I=((zzf)>=m^k*q)&((zzf)<m^(k+1)*q);
    nkp(k+1)=length(zzf(I));
    I=((zzf)<=-m^k*q)&((zzf)>-m^(k+1)*q);
    nkn(k+1)=length(zzf(I));
end
nk=nkp+nkn;
nratio=nk(1)./nk(2:end);
pos=1:N;
alphaz1=log(nratio)./(pos*log(m));
alphaz0=sum(alphaz1)/N;
% nratio=nk(1:end-1)./nk(2:end);
% alpha1=log(nratio)./(log(m));
% alpha0=sum(alpha1)/N;

npnratio=sum(nkn)/sum(nkp);
betaz0=(1-npnratio)/(1+npnratio);

kalphaz=alphaz0*(1-alphaz0)/(cos(pi*alphaz0/2)*gamma(2-alphaz0));
pos=0:N;
sigmakz=q*m.^pos.*(nk*alphaz0/(h*Nz*kalphaz*(1-m^(-alphaz0)))).^(1/alphaz0);
sigmaz0=sum(sigmakz)/(N+1);
...
Ncoef=2;
I=(abs(zxf)<epsilong)&(abs(zyf)<epsilong)&(abs(zzf)<epsilong);
zxinitial=zxf0(I);
zyinitial=zyf0(I);
zzinitial=zzf0(I);
x=zxf(I);
...
if alphax0==1
    ...
    Bx=n/Nz*x'/h-sigmax0^alphax0*betax0*epsilong^(1-alphax0)*kalphax/(1-alphax0);
end
if alphay0==1
    By=n/Nz*y'/h;
else
    By=n/Nz*y'/h-sigmay0^alphay0*betay0*epsilong^(1-alphay0)*kalphay/(1-alphay0);
end
if alphaz0==1
    Bz=n/Nz*z'/h;
else
    Bz=n/Nz*z'/h-sigmaz0^alphaz0*betaz0*epsilong^(1-alphaz0)*kalphaz/(1-alphaz0);
end
Bxx=n/Nz*x.^2'/h-sigmax0^alphax0*epsilong^(2-alphax0)*kalphax/(2-alphax0);
Byy=n/Nz*y.^2'/h-sigmay0^alphay0*epsilong^(2-alphay0)*kalphay/(2-alphay0);
Bzz=n/Nz*z.^2'/h-sigmaz0^alphaz0*epsilong^(2-alphaz0)*kalphaz/(2-alphaz0);
Bxy=n/Nz*(x.*y)'/h;
Bxz=n/Nz*(x.*z)'/h;
Byz=n/Nz*(y.*z)'/h;
nbinsq=10;
